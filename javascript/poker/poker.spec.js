...
describe('Poker', () => {
  ...
  test('royal flush win', () => {
    const hands = ['4D 5S 6S 8D 3C', '10H JH QH KH AH', '3S 2S 5D 6H JH'];
    const expected = ['10H JH QH KH AH'];
    expect(bestHands(hands)).toEqual(expected);
  });

  test('2 royal flush wins', () => {
    const hands = ['4D 5S 6S 8D 3C', '10H JH QH KH AH', '3S 2S 5D 6H JH', 'AH 10H KH JH QH'];
    const expected = ['10H JH QH KH AH', 'AH 10H KH JH QH'];
    expect(bestHands(hands)).toEqual(expected);
  });
